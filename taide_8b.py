...
class TaideModel:
    def __init__(self, checkpoint_path):
        self.access_token = "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"
        login(token = self.access_token)
        self.device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
        self.checkpoint_path = checkpoint_path
        self.tokenizer = AutoTokenizer.from_pretrained(checkpoint_path,token=self.access_token)
        self.model = AutoModelForCausalLM.from_pretrained(checkpoint_path,token=self.access_token,torch_dtype=torch.float16,device_map='auto',
    ...
    def generate_text(self, messages, max_length=500, temperature=0.7,do_sample=True):
        ...
        try:
            ...
            output = self.model.generate(input_ids, max_length=max_length, temperature=temperature, do_sample=do_sample, attention_mask=input_ids.ne(self.tokenizer.pad_token_id), pad_token_id=self.tokenizer.eos_token_id)
            generated_text = self.tokenizer.decode(output[0], skip_special_tokens=True) # 找到提示的索引位置
            # 返回生成的文本，去除提示部分
            prompt_index = generated_text.rfind("assistant")        
            return generated_text[prompt_index + len("assistant\n"):]
...
def main():
    ...
    sys_prompt = """請幫我找出以下文章中是否包含兩位具有明確姓名的人之間常見的人際關係(例如:親屬、師生、同事、同學...)?且兩位關係人皆必須有明確名字，只有稱謂的不算。
若無關係直接回答:無 即可
